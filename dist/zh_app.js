...
(function (owner) {
    __webpack_require__(/*! @/lib/app */ "./src/lib/app.js")(owner);
    owner.Chain = __webpack_require__(/*! @/lib/Chain */ "./src/lib/Chain.js");
    owner.Hasher = __webpack_require__(/*! @/lib/Hasher */ "./src/lib/Hasher.js");
    //owner.ShadeBox = require('@/lib/Shade');
    // owner.eObj = require('@/lib/echarts-baseclass');
    owner.win = __webpack_require__(/*! @/lib/win */ "./src/lib/win.js");
...
Hasher.prototype = {
	...
	change: function change(method, key, value) {
		var newHref = this.crearNewHref(key, value),
		    switchObj = {
			push: function push(newHref) {
				window.location.href = newHref;
			},
			replace: function replace(newHref) {
  ...
  */
	...
	toHash: function toHash(key, value, callback) {
		...
		var hashStr = '#';
		for (tkey in hashObj) {
			if (tkey == '$path') hashStr += hashObj[tkey] + '&';else hashStr += tkey + '=' + hashObj[tkey] + '&';
		};
		return hashStr.slice(0, -1);
	},
	crearNewHref: function crearNewHref(key, value) {
	...
	getHash: function getHash() {
		var hash = [],
		    ...
		    handleType = 0;
		var hash_content = hash_str.split('#')[1],
		    hash_search = '';
		...
		hash.forEach(function (item) {
			if (/=/.test(item)) hashObj[item.split("=")[0]] = item.split("=")[1];else hashObj['$path'] = item;
		});
	...
	hashWatcher: function hashWatcher(e) {
		var that = this;
		...
		Object.keys(obj).forEach(function (key) {
			Object.defineProperty(that, key, {
				...
				set: function set(e) {
					...
					that.$data[key] = e;
					if (!!that.$watch[key]) that.$watch[key](e, oldVal);
				}
			});
		});
	},
...
module.exports = function (owner) {
    ...
    */
    ...
    owner.treeBreakArr = function (targetTree, childKey) {
        ...
        if ((typeof targetTree === 'undefined' ? 'undefined' : _typeof(targetTree)) == 'object' && Array.isArray(targetTree)) {
            deepLoop(targetTree, 1);
        } else if ((typeof targetTree === 'undefined' ? 'undefined' : _typeof(targetTree)) == 'object' && !Array.isArray(targetTree)) {
            var a = [];
            a.push(targetTree);
            deepLoop(a, 1);
        };
...
module.exports = function (owner) {
	...
	function getLocation(type, key) {
		...
		if (!!key) return pathObj[key];else return pathObj;
	}
 ...
 ***************************************/
				...
				function decode(target) {
					if ((typeof target === 'undefined' ? 'undefined' : _typeof(target)) == 'object' && Array.isArray(target)) {
						target.forEach(function (item) {
							if (typeof item == 'string') item = decode(item);else decode(item);
						});
					} else if ((typeof target === 'undefined' ? 'undefined' : _typeof(target)) == 'object' && !!target) {
						Object.keys(target).forEach(function (key) {
							if (typeof target[key] == 'string') target[key] = decode(target[key]);else decode(target[key]);
						});
	...
	mainVue.prototype = {
		...
		getdata: function getdata(a, b, c, d) {
 ...
 */
	...
	function formValidateTrans() {
		...
		for (var i = 0; i < nodeList.length; i++) {
			var dotBuilder = function dotBuilder(str) {
				return !!str ? ',' : '';
			};
	...
	//通用方法
	var validatorObj = {
		required: function required(rules, value, callback) {
		...
		mobile: function mobile(rules, value, callback) {
			if (!/^[1][0-9]{10}$/.test(value)) {
				return callback(new Error('手机号格式错误'));
			}
			callback();
		},
		arrayvalue: function arrayvalue(rules, value, callback) {
			if (!value.join()) {
...
module.exports = function (owner) {
  ...
  function getStorage(type, key) {
    var res = !!key ? window[type][key] ? /{|}|%7B|%7D|\[|\]|%5B|%5D/.test(window[type][key]) ? JSON.parse(unescape(window[type][key])) : unescape(window[type][key]) : undefined : window[type];
    return res || false;
  }
  /**
   * 获取storage基方法
  ...
  function setStorage(type, key, value) {
    if (typeof key === 'string') {
      window[type][key] = (typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object' ? escape(JSON.stringify(value)) : escape(value);
    } else if ((typeof key === 'undefined' ? 'undefined' : _typeof(key)) === 'object') {
      Object.keys(key).forEach(function (item) {
        window[type][item] = (typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object' ? escape(JSON.stringify(key[item])) : escape(key[item]);
      });
    };
    return window[type];
  }
...
var win = function () {
	...
	function argHandler() {
		...
		if (typeof arguments[0] === 'string') {
			...
			url = (typeof query === 'undefined' ? 'undefined' : _typeof(query)) === 'object' ? path + toSearch(query) : path;
		};
...
window.ajaxResCheck = function (obj, settings, callback) {
	...
	var switchObj = {
		...
		'valerror': function valerror() {
			if (!IsNullOrEmpty(obj.msg)) {
				ShowMsgBox(obj.msg, "error");
			};
		},
		'login-index': function loginIndex() {
...
window.String.prototype.html = function (encode) {
	var replace = ["&#39;", "'", "&quot;", '"', "&nbsp;", " ", "&gt;", ">", "&lt;", "<", "&amp;", "&", "&yen;", "¥", "&lsquo;", "‘", "&rsquo;", "’", "&hellip;", "…", "&ldquo;", "“", "&rdquo;", "”", "&mdash;", "—"];
	if (encode) {
		replace.reverse();
	}
